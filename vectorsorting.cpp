...
int partition(vector<Bid>& bids, int begin, int end) {
    // variables to support partitioning for quicksort
    int low = 0;
    int high = 0;
    int midpoint = 0;
    string pivot; // the point of partition
    Bid temp;
    bool done = false;
    ...
    while (!done) {
        ...
        // incrementing low index while bids[low] < bids[pivot]
        while (bids.at(low).title.compare(pivot) < 0) {
            ++low;
        }

        // decrementing high index while bids[pivot] < bids[high]
...
void selectionSort(vector<Bid>& bids) {
    ...
    int lowestAlphaTitle = 0; //define min as int (index of the current minimum bid)
    ...
    for (unsigned int i = 0; i < bids.size(); ++i) {
        ...
        for (unsigned int j = i + 1; j < bids.size(); ++j) {
            ...
            // Compare object at j index with lowestAlphaTitle
            if (bids.at(j).title.compare(bids.at(lowestAlphaTitle).title) < 0) {
                lowestAlphaTitle = j; // if this element's title is less than minimum title
...
int main(int argc, char* argv[]) {
    ...
    while (choice != 9) {
        ...
        case 3:
            // Initialize timer variable before sorting
            ticks = clock();

            // selectionsort the bids
            selectionSort(bids);
